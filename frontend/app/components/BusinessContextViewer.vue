...
import { FileText, Copy, Check, Briefcase } from 'lucide-vue-next'
...
async function copyToClipboard() {
  if (businessContext.value?.content) {
    await navigator.clipboard.writeText(businessContext.value.content)
    copied.value = true
    setTimeout(() => {
      copied.value = false
    }, 2000)
    ...
    <SheetContent side="right" class="w-full sm:max-w-2xl lg:max-w-4xl overflow-hidden flex flex-col">
      <SheetHeader class="flex-shrink-0">
        <div class="flex items-center gap-3">
          <div class="rounded-lg p-2 border bg-primary/10 text-primary border-primary/20">
            <Briefcase class="h-5 w-5" />
          </div>
          <div class="flex-1 min-w-0">
            <SheetTitle class="text-xl truncate">Business Context</SheetTitle>
      ...
      <ScrollArea class="flex-1 -mx-6 px-6">
        ...
        <div v-else-if="status === 'error'" class="text-center py-12 text-muted-foreground">
          ...
          <p>Failed to load business context</p>
        </div>

        <div v-else-if="businessContext" class="prose dark:prose-invert pb-6">
          <div v-html="renderMarkdown(businessContext.content)" />
        </div>
      </ScrollArea>
    </SheetContent>
  </Sheet>
